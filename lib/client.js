window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // is the keyed slot `plugins.row.config`, declared by the official
    // plugin-manager page, and that page renders a row's configure control only
    // while the EXACT key below sits on the slot's registration ledger:
    //
    //     rowConfigKey(pkg.name, row.rowId) → `${pkg.name}#${row.rowId}`
    //     has: (row) => ledger.rows.has(rowConfigKey(pkg.name, row.rowId))
    //
    // So the key IS the contract: the package name from package.json plus the row
    // id this package's own cordis.patch.yml declares. It lives in one constant and
    // is exported for inspection; test/lifecycle-source.test.mjs re-derives both
    // halves from those two files and fails on any drift.
    const ROW_CONFIG_KEY = 'dsh-subagent-conductor#subagent-conductor'
    ...
    // resolved object is used unchanged by the selector and the settings card.
    ...
    // This plugin's settings namespace and profile entry id are both
    // 'subagent-conductor', so one key resolves on either host.
    ...
    const CSS = [
      ...
      // ── the card owns the settings-column typography ────────────────────────
      //
      // The sibling YOTK settings cards pin EVERY text node to the official scale:
      // dsh-mcp-pill `.dmpLabel{font-size:13px;font-weight:500}`, `.dmpHint{font-size:12px}`;
      // dsh-tool-adapt `.dtaLabel{font-size:13px;font-weight:500}`, `.dtaHint{font-size:12px}`.
      // This card was authored for a wider plugin surface and pinned only its own
      // title/description, leaving the body to INHERIT the shell base. The settings
      // panel is portaled to <body> (ui-settings-general `createPortal(…, document.body)`),
      // and no shipped stylesheet sets html/body font-size, so the inherited base
      // there is the UA default 16px: field labels, the 路由优先级 paragraph, role
      // rows, hints, the status line and the inline dialogs all rendered at 16px —
      // LARGER than this card's own 15px title, and larger than every sibling. The
      // card ROOT owns the fix so every unpinned descendant lands on the sibling
      // scale at once, declared with the official `--dsw-font-*` semantic tokens
      // (theme-presenter publishes them on <body>) and the siblings' own px as the
      // fallback for a host that predates them. Scoped to `.dscCard`: the composer
      // Selector shares `.dscHint`/`.dscError` but never renders inside the card.
      '.dscCard{font:inherit;font-size:var(--dsw-font-xs-13-font-size,13px);line-height:var(--dsw-font-xs-13-line-height,1.5);color:inherit;list-style:none;border:1px solid var(--dsw-alias-border-l2);border-radius:12px;background:var(--dsw-alias-bg-layer-3);transition:border-color .16s,background .16s}.dscCard:hover,.dscCardOpen{border-color:var(--dsw-alias-label-dimmed)}.dscCardOpen{background:var(--dsw-alias-bg-layer-2)}',
      // Sibling field labels / emphasised card text are 13px weight 500.
      '.dscCard .dscFieldLabel,.dscCard strong{font-weight:var(--dsw-font-xs-strong-13-font-weight,500)}',
      // Sibling hints and failure/secondary copy are 12px.
      '.dscCard .dscHint,.dscCard .dscError{font-size:var(--dsw-font-xxs-12-font-size,12px);line-height:var(--dsw-font-xxs-12-line-height,1.5)}',
      ...
      '.dscRowSummary{color:var(--dsw-alias-label-tertiary);font-size:13px;line-height:1.5}',
    ].join('')
    ...
    const SOURCE_LABELS = { role: '默认角色', session: '根会话选择', default: '全局默认', inherit: '官方/原生' }
    ...
    function normalizeImportedRoles(raw) {
      try {
        ...
        for (const [id, record] of Object.entries(raw)) {
          ...
          roles[id] = { displayName, description, ...(provider ? { provider } : {}), ...(model ? { model } : {}), ...(reasoningEffort ? { reasoningEffort } : {}) }
        }
        return { roles }
      } catch (error) {
    ...
    function SettingsCard({ scope, api, defaultOpen }) {
      const [, tick] = React.useReducer((n) => n + 1, 0)
      const [draft, setDraft] = React.useState(null)
      const [message, setMessage] = React.useState('')
      const [groups, setGroups] = React.useState([])
      // The disclosure default belongs to the SEAT, not to the card. Two seats
      // render this one card alone on a page of its own — the row seat's
      // `view === 'page'` branch and the additive `settings.section` page — and
      // both pass `defaultOpen: true`. Collapsed stays the default for the legacy
      // ≤ 0.1.5 `settings.plugin.item` seat, because there the card still sits in
      // the Plugins list beside many other cards. The header button keeps folding
      // either one back up, so `defaultOpen` only seeds the initial state.
      ...
      const form = draft || { provider: value.defaultRoute?.provider || '', model: value.defaultRoute?.model || '', effort: value.defaultRoute?.reasoningEffort || '', defaultRole: value.defaultRole || '', roles: value.roles || {} }
      ...
      async function save() {
        try {
          // defaultRoute is written LEAF BY LEAF. Its Config leaves are volatile
          // but the enclosing node is not, and the declarative host refuses any
          // op whose path is not beneath a volatile node ("Config field
          // defaultRoute is not volatile"), so a whole-object write at
          // ['defaultRoute'] never lands there. Per-leaf set/unset is also
          // accepted by the ≤ 0.1.5 namespace mutation, which this bundle already
          // drives with nested paths (sessionSelections.<rootId>).
          const route = { provider: form.provider.trim(), model: form.model.trim(), reasoningEffort: effortAvailable && form.effort.trim() ? form.effort.trim() : '' }
          const ops = [
            ...['provider', 'model', 'reasoningEffort'].map((field) => (route[field]
              ? { op: 'set', path: ['defaultRoute', field], value: route[field] }
              : { op: 'unset', path: ['defaultRoute', field] })),
      ...
      const openRoleEditor = (id, role, copy) => {
        ...
        setDialogError('')
        setRoleEditor({ originalId: copy ? null : (id || null), id: nextId, displayName: source.displayName || '', description: source.description || '', provider: source.provider || '', model: source.model || '', reasoningEffort: source.reasoningEffort || '' })
      }
      const saveRoleEditor = () => {
        ...
        const role = { displayName: roleEditor.displayName.trim(), description: roleEditor.description.trim(), ...(roleEditor.provider && roleEditor.model ? { provider: roleEditor.provider, model: roleEditor.model } : {}), ...(roleEditor.provider && roleEditor.model && roleEffortAvailable && roleEditor.reasoningEffort ? { reasoningEffort: roleEditor.reasoningEffort } : {}) }
      ...
      const rolesField = e('section', { className: 'dscField' },
        ...
        e('label', { className: 'dscField' }, e('span', { className: 'dscFieldLabel' }, '默认角色'), e('select', { value: form.defaultRole, onChange: (event) => set('defaultRole', event.target.value) }, e('option', { value: '' }, '不指定'), roleEntries.map(([id, role]) => e('option', { key: id, value: id }, `${role.displayName} · ${id}`))), e('span', { className: 'dscHint' }, '默认角色路由低于输入框根会话选择；仅在没有会话选择时生效。')),
        e('div', { className: 'dscRoleList' }, roleEntries.length ? roleEntries.map(([id, role]) => {
          ...
          const route = role.provider && role.model ? `${model?.name || role.model} · ${role.provider}${role.reasoningEffort ? ` · ${role.reasoningEffort}` : ''}` : '继承官方选择或全局默认'
        ...
        }) : e('p', { className: 'dscRoleEmpty' }, '还没有角色。新建角色并设为默认角色后，其路由会在没有根会话选择时作用于子代理。')),
      )
      ...
      const jsonDialog = jsonOpen && e('section', { className: 'dscInlinePanel', role: 'region', 'aria-label': '导入或导出角色 JSON' },
          ...
          e('p', null, '高级入口：可复制当前 JSON，或粘贴完整角色对象后应用到草稿。每条记录都会先校验（ID、必填字段、provider/model 配对），最终保存仍由 Host 校验。'),
        ...
        e('li', { className: `dscCard${open ? ' dscCardOpen' : ''}` },
          ...
          open && e('div', { className: 'dscCardBody' },
            e('div', { className: 'dscStack' },
              e('section', { className: 'dscField' }, e('strong', null, '路由优先级'), e('p', { className: 'dscPriority' }, '输入框根会话选择 ＞ 默认角色路由 ＞ 本页全局默认 ＞ 官方工具显式选择 ＞ DSH 原生继承。provider、model 和思考强度分别按此顺序解析；最终组合仍会校验，配置无效时保持官方行为不变。')),
              modelRouteField,
    ...
    function apply(ctx) {
      ...
      // is no longer available on the Client, so there is no fallback branch.
      const remote = ctx.get('remote')
      const api = remote
        ? {
            llm: { models: () => remote.session.modelCatalog().then((result) => ({ result })) },
            settings: { mutate: (payload) => remote.settings.mutate(payload.ns, payload.ops, payload.expectedRevision).then((result) => ({ result })) },
          }
        : undefined
      // Version-portable scope. Both transport names are awaited WITHOUT gating
      // activation, and whichever this host mounts resolves; see
      // resolveSettingsScopeFrom.
      let scope
      // The SELECTOR uses the scope only for extra display/state; it must mount
      // on the bare context so the composer is never blocked by the settings
      // transport. DSH 0.1.2 Slot contract supplies standard session-scoped props
      // directly; do not use the removed ownerProps/sessionId injection shape.
      ctx.slots.inject('conversation.input.right', () => ctx.slots.register(
        { name: 'conversation.input.right', id: NS, order: -10 },
        (props) => e(Selector, { ...props, api, scope }),
      ))
      // The Settings card MUST register from inside a context that has the
      // settings transport, i.e. from the `ctx.inject([...], cb)` callback and on
      // the CHILD context it hands us. Registering `settings.plugin.item` from
      // the bare apply context puts the entry where the Plugins tab's ledger
      // never sees it: the tab reads `ctx.slots.entries('settings.plugin.item')`
      // from its own context, gets an empty list, and the card never renders
      // even though `register` returned normally. (dshmarket, an out-of-repo
      // bundle that does render, uses this nested shape.)
      const registerCard = (sctx) => {
        ...
        sctx.slots.inject('settings.plugin.item', () => sctx.slots.register(
          { name: 'settings.plugin.item', key: NS, label: 'CONDUCTOR' },
          () => e(SettingsCard, { scope, api }),
        ))
      }
      ...
      // version sniffing: `slots.inject` fires only where its own slot is
      // declared and disposes the entry when that declaration collapses.
      //
      ...
      // `props.form` (the manager's own ConfigPageForm) is deliberately NOT
      ...
      // this plugin keeps exactly one read path and one write path. `scope` is
      // read from the enclosing apply closure at render time, exactly like the
      // legacy card registration above.
      //
      // `slots` is awaited NON-GATINGLY here, and the callback returns the
      // registration disposer so the entry is owned by the registering fiber and
      // released with it on stop / update / HMR.
      const registerRowConfig = (sctx) => sctx.slots.inject('plugins.row.config', () => sctx.slots.register({
        name: 'plugins.row.config',
        key: ROW_CONFIG_KEY,
      }, function ConductorRowConfig(props) {
        if (props && props.view === 'summary') {
          return e('span', { className: 'dscRowSummary' }, '全局默认路由与角色模板；候选项来自主模型目录。')
        }
        // view === 'page': this card IS the row's configuration page, so it starts
        // expanded — the collapsed default belongs to the legacy Plugins-list seat.
      ...
      // disposer, and cordis collects a callback's returned function as an effect
      // of the child fiber it creates (`Fiber._execute`), so the entry is owned by
      // the registering fiber and released with it on stop / update / HMR exactly
      // like the row seat. The seat is host-version dependent: a host that does not
      // declare it simply never fires the inner `slots.inject`, so this
      // registration can never gate the plugin.
      const registerSettingsSection = (sctx) => sctx.slots.inject('settings.section', () => sctx.slots.register({
        name: 'settings.section',
        id: 'yotk-subagent-conductor',
        order: 62,
        label: () => 'YOTK · Conductor',
      }, function ConductorSettingsSection() {
        // The section owner shares `close` and nothing else; this card needs
        // neither `close` nor the host-owned optional `form` prop, because values
        // keep flowing through the one resolved transport — so the page branch is
        // literally the row seat's `view === 'page'` render, including
        // `defaultOpen: true`: one card alone on the page.
        return e(SettingsCard, { scope, api, defaultOpen: true })
      }))
    ...
    // 'slots' is not a version-dependent service: every Web host provides it, and
    // it orders the slot registrations in apply after the registry exists.
