...
export function apply(ctx) {
  ...
  ctx.inject(['settings'], (sctx) => {
    // ── settings namespace: dual-host registration ──────────────────────────
    // ≤ 0.1.5  ctx.settings.register(namespace, schema, options) exists and this
    //          plugin owns its `subagent-conductor` namespace.
    // ≥ 0.1.7  register() is GONE: the settings service derives one form per
    //          ACTIVE profile entry from that entry's Config schema, and only
    //          `.volatile()` fields are editable (no `volatile` exists on the
    //          0.1.5 schemastery line, so one schema expression cannot serve
    //          both hosts).
    // On the newer host the entry Config is the storage instead. Routing must
    // keep working: `getSettings` already degrades to normalized-empty, which is
    // exactly the documented "no settings yet" contract for this listener.
